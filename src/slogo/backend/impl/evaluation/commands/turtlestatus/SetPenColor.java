...
public class SetPenColor extends Operation {
    ...
    protected IExecutionContext executeRaw (List<IExecutionContext> args,
        ...
        int red = Integer.parseInt(args.get(0).environment().get(Constants.RETURN_VALUE_ENVIRONMENT));
        int green = Integer.parseInt(args.get(1).environment().get(Constants.RETURN_VALUE_ENVIRONMENT));
        int blue = Integer.parseInt(args.get(2).environment().get(Constants.RETURN_VALUE_ENVIRONMENT));
        Color c;
        try{
        ...
        Qualities q = new Qualities(c, status.turtleQualities().index(), status.turtleQualities().thickness());
