...
public abstract class ConditionalOperation extends Operation{
	 ...
	 */
	public IExecutionContext kipling(List<IExecutionContext> args,IExecutionContext previous, ISyntaxNode current,
			int conditionIndex, int startOfCommands) throws MalformedSyntaxException {
		int condition;
		...
		if (conditionString.length() != 0) {
			...
			throw new MalformedSyntaxException("Condition expression could not be found at position: " + conditionIndex);
		}
	...
	private int getMatchingParentheses(List<ISyntaxNode> nodes, int openingIndex) throws MalformedSyntaxException{
