...
public abstract class LoopOperation extends Operation {
    ...
    protected IExecutionContext loop (List<IExecutionContext> args, IExecutionContext previous,
            ISyntaxNode current, int countStartIndex, int countEndIndex, int incrementIndex,
            int counterNameIndex, int startOfCommands) throws MalformedSyntaxException {
        int countStart = handleCountStart(countStartIndex, args);
        int countEnd = Integer.parseInt(getContextReturnValue(args.get(countEndIndex)));
    ...
    private int handleCountStart (int countStartIndex, List<IExecutionContext> args)
        ...
        if (countStartIndex != -1) {
            ...
            if (retVal.length() != 0) {
                ...
                throw new MalformedSyntaxException(
                        "The starting index for the loop could not be found at position: "
                                + countStartIndex);
            }
        }
        return countStart;
    ...
    private int handleIncrement (int incrementIndex, List<IExecutionContext> args)
            throws MalformedSyntaxException {
        int increment = 1;
        if (incrementIndex != -1) {
            ...
            if (retVal.length() != 0) {
                ...
                throw new MalformedSyntaxException(
                        "The increment index for the loop could not be found at position: "
                                + incrementIndex);
            }
        }
        return increment;
    ...
    private String handleCounterName (int counterNameIndex, List<IExecutionContext> args)
            throws MalformedSyntaxException {
        ...
        if (counterNameIndex != -1) {
            ...
            if (varName.length() != 0) {
                ...
                throw new MalformedSyntaxException(
                        "The counter variable name for the loop could not be found at position: "
                                + counterNameIndex);
            }
        }
        return counterName;
