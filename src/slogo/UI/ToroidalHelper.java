...
public class ToroidalHelper {
	...
	public List<Line> makeLinesToroidal() {
		...
		for(Line l: lineList) {
			if (!checkEndBounds(l)) {
				...
				double angle = Math.atan2(l.getEndX() - l.getStartX(), -(l.getEndY() - l.getStartY()));
				if (angle > Math.PI/2) angle = angle - Math.PI / 2;
				else if (angle < -1 * Math.PI/2) angle = angle + Math.PI / 2;
	...
	private void runXCalculations(Line l, double angle, List<Line> lines) {
		if (checkXBounds(l.getEndX())) lines.add(l);
	...
	private void runYCalculations(Line l, double angle, List<Line> lines) {
		if (checkYBounds(l.getEndY())) lines.add(l);
		else if (l.getEndY() > MAX_Y) {
			Line newLine = new Line();
			setLineCoords(newLine, newLine.getStartX() - (MAX_Y - newLine.getStartY()) * Math.tan(angle), MIN_Y, newLine.getEndX(), newLine.getEndY() - HEIGHT);
			setLineCoords(l, l.getStartX(), l.getStartY(), newLine.getStartX() - (MAX_Y - newLine.getStartY()) * Math.tan(angle), MAX_Y);
			lines.add(l);
	...
	private boolean checkStartBounds(Line l) {
		return (l.getStartX() <= MAX_X && l.getStartX() >= MIN_X && l.getStartY() <= MAX_Y && l.getStartY() >= MIN_Y);
	}
