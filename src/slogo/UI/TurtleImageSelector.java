...
public class TurtleImageSelector {
    ...
    protected void create (Group root, Turtle turtle, View view) {
        ...
        sc.setUpSelector("Turtle Image", AppConstants.SELECTOR_WIDTH, AppConstants.SELECTOR_HEIGHT,
                         AppConstants.SELECTOR_FONT_SIZE, myPossibleImages);
        mySelector = sc.getSelector();
        mySelector.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle (ActionEvent event) {
                if (mySelector.getValue() != null) {
                    view.sendCommandToBackend("SetShape " +
                            (mySelector.getItems().indexOf(mySelector.getValue()) + 1));
                }
            }
        });
        VBox selectorWithLabel =
                sc.createSelectorWithLabel("Select a Turtle Image", AppConstants.LABEL_FONT_SIZE,
                                           Color.BLACK);
